...
const createPlan = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Plan created successfully", success: true, savedPlan });
...
const getAllPlans = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: `Plans could not be collected successfully:${error}` });
  }
};
...
const updatePlan = async (req, res) => {
  try {
    ...
    if (!plan) {
      return res
        .status(404)
        .json({ message: "Plan not found or does not exist" });
    }
